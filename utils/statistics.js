...
export function calculateStatistics(transactions) {
    ...
    const calculateStandardDeviation = (arr) => {
      ...
      const sumDifferences = arr.reduce((acc, num) => acc + Math.pow(num - mean, 2), 0);
      return Math.sqrt(sumDifferences / arr.length);
    };
  
    const totalIncome = incomes.reduce((a, b) => a + b, 0);
    const totalExpenses = expenses.reduce((a, b) => a + b, 0);
    const incomeExpensePercentage = totalIncome ? (totalExpenses / totalIncome) * 100 : 0;
    ...
    const expenseDistribution = Object.fromEntries(
      Object.entries(expenseCategories).map(([key, value]) => [key, (value / totalExpense) * 100])
    );
